...
export async function GET(req: NextRequest) {
  ...
  if (!admin) {
    return NextResponse.json({ error: 'forbidden', message: 'Nur für Administratoren.' }, { status: 403 });
  }
...
export async function PATCH(req: NextRequest) {
  ...
  if (!userId || !ALLOWED.has(status)) {
    return NextResponse.json(
      { error: 'invalid_input', message: 'userId und ein gültiger Status sind erforderlich.' },
      { status: 400 },
    );
  }
